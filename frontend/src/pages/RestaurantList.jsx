...
const googleMapsApiKey = getEnv('VITE_MAPS_API');
        ...
        <div className="restaurant-list">
          {restaurants.length === 0 ? (
            ...
            restaurants.map((restaurant, index) => (
              <div key={restaurant._id} className="restaurant-box" data-cy={`restaurant-box-${index}`}>
                <h3 data-cy={`restaurant-name-${index}`}>{restaurant.name}</h3>
                <p data-cy={`restaurant-address-${index}`}>{restaurant.address.street}, {restaurant.address.city}, {restaurant.address.state} {restaurant.address.zip}</p>
                <p data-cy={`restaurant-contact-${index}`}>Contact: {restaurant.contact}</p>
                <Link to={`/restaurants/${restaurant._id}/menu`} data-cy={`view-menu-${index}`} className="view-menu-link">View Menu</Link>
              </div>
            ))
          )}
        </div>
